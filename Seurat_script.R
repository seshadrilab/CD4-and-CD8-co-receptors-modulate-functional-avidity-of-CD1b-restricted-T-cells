...
d <- read.csv(file.path(projectDir, "/data/alpha-pheno-mapped_FINAL.csv", header=T, na.strings=c("NA", "null", "<NA>", ""))
dput(colnames(d)) 
...
d <- read.csv(file.path(projectDir, "/data/alpha-pheno-mapped_FINAL.csv", header=T, na.strings=c("NA", "null", "<NA>", ""))dput(colnames(d))
